        ...
        if app_functionality == "Chat":
            ...
            if chat_type == "Q&A with stored SQL-DB":
                ...
                if os.path.exists(APPCFG.sqldb_directory):
                    ...
                    # Extract only the SQL from the generated text
                    def extract_sql_only(output: str) -> str:
                        match = re.search(r"SQLQuery:\s*(.*)", output)
                        return match.group(1).strip() if match else output
                    chain = (
                        RunnablePassthrough
                        .assign(query=write_query)
                        .assign(query=RunnableLambda(lambda x: extract_sql_only(x["query"])))  # Extract pure SQL
